...
class Emitter:
    """可跨进程 pickle 的产出器：捕获 pid 与事件总线队列。"""

    def __init__(self, pid, event_bus):
        self.pid = pid
        self.event_bus = event_bus

    def __call__(self, event):
        event["source"] = self.pid
        self.event_bus.put(event)


class ProcessHandle:

    def __init__(self, pid, spawn, event_bus, lazy=False):
        self.pid = pid
        self.spawn = spawn
        self.emit = Emitter(pid, event_bus)
