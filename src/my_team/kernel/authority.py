...
class Authority(KernelModeDevice):
    ...
    def _build_inject(self, agent: str) -> dict:
        ...
        Device identities are globally visible according to ordinary position
        grants; there is no per-agent instance namespace.
        """
        ...
        for dev_id, dev in self._identities.items():
            if dev["agent"] or dev_id not in by_device:
                continue
