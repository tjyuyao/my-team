...
6. **沙箱与权限解耦（v0.14 修订目标）**。FS 层（挂载矩阵）与 Authority
   （业务布线）**分离**：正常挂载矩阵只给进程自己的家
   （`data/<id>`）与只读系统；设备实现也属于该设备家，框架不设共享源码区、
   不扫描 agent 目录，也不提供 `per-agent` 设备或 `bound_agent` 绑定。
   设备进程永远不是 root。设备维护是显式的、按目标设备限定的沙箱会话：
   设备卸载后，获授权维护者可额外挂载该设备家进行编辑，再由 kernel 裁决
   `install_device` 重新加载；此例外不扩展其它身份或 Authority 数据权限。
   跨区数据访问仍走调用级裁决（目标设备按发起 Agent 的权限）。
