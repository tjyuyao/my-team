...
class RecordSchema(BaseModel):
    ...
    def _validate(self) -> "RecordSchema":
        ...
        for uf in self.unique_fields:
            if uf not in names:
                raise ValueError(
                    f"unique field '{uf}' not declared in fields"
                )
        return self
...
class RecordStore(Device):
    ...
    def register_schema(self, schema: RecordSchema) -> None:
        ...
        if schema.record_type in self._schemas:
            raise ValueError(
                f"record type '{schema.record_type}' already registered"
            )
    ...
    def schema(self, record_type: str) -> RecordSchema:
        try:
            ...
            raise RecordInvariantError(
                record_type, "", "record type not registered",
            ) from None
    ...
    def replay(self) -> dict[str, dict[str, Any]]:
        ...
        for entry in self._ledger:
            ...
            if entry.op == "upsert":
                ...
                current[entry.delta_field or ""] = (
                    current.get(entry.delta_field or "", 0)
    ...
    def upsert(
        ...
        self._version_counter[full_key] = (
            self._version_counter.get(full_key, 0) + 1
        )
        return MutationResult(
            record_type=record_type, key=key, record=record,
    ...
    def apply_delta(
        ...
        if existing is None:
            existing = {
                spec.name: (
                    "" if spec.type == "string"
                    else 0 if spec.type in {"int", "float"}
                    else False if spec.type == "bool"
                    else ""
                )
    ...
    def invert_mutation(
        ...
        self._ledger[:] = [
            e for e in self._ledger if e.ledger_id not in ledger_ids_set
        ]
        full_key = f"{record_type}:{key}"
