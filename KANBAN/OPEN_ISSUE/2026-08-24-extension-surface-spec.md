...
> `KANBAN/TODO/2026-08-24-package-trust-boundary.md`（E5）决策记录节。
>
> **【追加·同日】场景包定位**：具备完全扩展能力的自包含安装单元，
> 可自带 ToolPlugin 定义的工具（与内核内置工具同权）；「对既有能力包
> 引用为主」表述作废，独立能力包为跨场景复用的可选形态。
>
> **【追加·同日】编排层坍缩 + 三态收敛**：**ProcessDef/ProcessInstance
> 废弃**——流程 = 业务语言 SOP 知识（SPEC §1.8），编排实例载体 =
> Task 树；「编排层」一节以下正文（声明式 ProcessDef/门原语）为
> **历史讨论**，不再构成设计权威。系统归三类：内核（纯逻辑）/ 设备
> （数据+读写工具+ACL+锁）/ Agent（内心/头脑/双手）；岗位模型
> （Position：JD/边/授权）、组织架构设备、记忆与注入系统、Task
> 设备、世界记忆设备（Journal）、配置设备见 SPEC §1.7/§4/§5/§6。
