...
## 验收标准
- [x] 回滚 tick 后 `pending_ops.pending_count == 0`（本 tick 新增部分）
- [x] 回滚后 agent 的 continuation 与 state 与 tick 开始前一致
- [x] 同一 request_id 在回滚后可重新提交
- [x] 连续 3 个回滚 tick 不累积 SUBMITTED op（复现 OI-003 P0-2 的实验转为测试）
