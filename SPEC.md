...
- **双层测试**：
  - **T1 逻辑层（默认门禁，亚秒）**：进程内/内核态直调——Authority、
    BucketDispatcher、事件校验、agent 决策、维护授权拒绝路径。零 spawn、
    零 bwrap；"两态同构"（用户态/内核态同契约，传输是唯一差异）保证逻辑
    层可脱离子进程验证。覆盖率以此层计。
  - **T2 集成层（`-m integration` 显式可选）**：真实子进程 + 沙箱，只留
    真正跨进程的不变量——设备事件往返（spawn→serve→respond→宿主读侧
    盖章→总线）、维护会话双锚点挂载、身份/授权边界。数量保持个位数。
  - 命令：默认 `uv run pytest -m "not integration"`；全量（含集成）
    `uv run pytest`（当前全量 18 测 ≈ 3s）。
- **只写行为测试**：断言可观察结果（进程存活、真实响应内容、授权裁决）；
  不写"空心"断言（如注册簿含身份却不验设备真跑通——曾掩盖维护流程四个
  bug）。
- **进程必须显式 terminate**：泄漏的非 daemon 子进程会在解释器退出时被
  `multiprocessing._exit_function` 无限 join，表现为"测试挂几分钟"。
